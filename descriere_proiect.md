...
- preturile istorice se inregistreaza periodic pe perechea (simbol, exchange, moment).

## 2) Constrangeri (restrictii, reguli)

- toate cheile primare sunt NOT NULL;
- urmatoarele coduri sunt unice: cod_exchange, ticker, cod_moneda;
...
- pe istoric_pret nu se permit duplicate pentru (ticker, id_exchange, moment_cotatie);

## 3) Entitati (cu cheia primara)

1. exchange (id_exchange)
2. companie (id_companie)
3. moneda (cod_moneda)
4. simbol_bursier (ticker)
5. investitor (id_investitor)
6. portofoliu (id_portofoliu)
7. detinere_portofoliu (id_portofoliu, ticker)  -- tabel asociativ
8. istoric_pret (id_istoric)
9. ordin (id_ordin)
10. executie_ordin (id_executie)
11. tranzactie_numerar (id_tranzactie)
12. curs_valutar (id_curs)
...
1. **exchange**
   - `id_exchange` (Primary Key)
   - `cod_exchange` (VARCHAR2(10), UNIQUE, NOT NULL) - ex: NYSE, DAX
   - `denumire` (VARCHAR2(100), NOT NULL)
   - `tara` (VARCHAR2(50), NOT NULL)
   - `fus_orar` (VARCHAR2(40))
...
7. **detinere_portofoliu** (tabel asociativ)
...
8. **istoric_pret**
   - `id_istoric` (Primary Key)
   - `ticker` (Foreign Key -> simbol_bursier.ticker, NOT NULL)
   - `id_exchange` (Foreign Key -> exchange.id_exchange, NOT NULL)
   - `moment_cotatie` (TIMESTAMP, NOT NULL)
   ...
   - Constrangere unica: (`ticker`, `id_exchange`, `moment_cotatie`)
...
## 5) Atribute importante si constrangeri cheie
...
Se va realiza ulterior, pe baza sectiunilor 3-5.
...
- EXCHANGE(`id_exchange` PK, `cod_exchange` UQ, `denumire`, `tara`, `fus_orar`)
...
- ISTORIC_PRET(`id_istoric` PK, `ticker` FK, `id_exchange` FK, `moment_cotatie`, `pret_deschidere`, `pret_maxim`, `pret_minim`, `pret_inchidere`, `volum`, UQ(`ticker`,`id_exchange`,`moment_cotatie`))
- ORDIN(`id_ordin` PK, `id_portofoliu` FK, `ticker` FK, `tip_sens`, `tip_ordin`, `cantitate`, `pret_limita`, `data_ordin`, `status_ordin`)
- EXECUTIE_ORDIN(`id_executie` PK, `id_ordin` FK, `id_exchange` FK, `cantitate_executata`, `pret_executie`, `data_executie`)
